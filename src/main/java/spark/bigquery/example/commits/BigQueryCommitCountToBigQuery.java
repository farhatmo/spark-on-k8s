...
package spark.bigquery.example.commits;
 ...
 */
public class BigQueryCommitCountToBigQuery {
  ...
  private static final String COMMITTER_COLUMN = "committer_name";
  private static final String COMMITTER_COUNT_COLUMN = "committer_count";
  private static final TableSchema OUTPUT_TABLE_SCHEMA = new TableSchema().setFields(
      ImmutableList.of(
          new TableFieldSchema().setName(COMMITTER_COLUMN).setType("STRING"),
          new TableFieldSchema().setName(COMMITTER_COUNT_COLUMN).setType("INTEGER")
      ));
  ...
  private static JsonObject toJson(Tuple2<String, Long> tuple) {
    ...
    jsonObject.addProperty(COMMITTER_COLUMN, tuple._1);
    jsonObject.addProperty(COMMITTER_COUNT_COLUMN, tuple._2);
    return jsonObject;
  }
}
